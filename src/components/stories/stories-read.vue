<template>
  <v-responsive color="primary">
    <v-container grid-list-xs>
      ...
      <v-data-table :headers="headers" :items="tasksArray" hide-actions class="elevation-1">
  ...
  methods: {
    getStory () {
      HTTP.get('/users/' + this.user.id + '/clients/' + this.client.id + '/epics/' + this.epic.id + '/stories/' + this.$route.params.id)
        .then(response => {
          this.setStory = response.data.story
          this.tasksArray = response.data.story.subtask
        })
        .catch(error => {
          this.snackbar = true
